...
action="/house/doupdate?hId=${house.HId}" enctype="multipart/form-data"  modelAtttribute="updateVO" >
<DIV class=infos>
<TABLE class=field>
  <TBODY>
  <input type="hidden" id="hId" name="hId" value="${house.HId}">
  <TR>
  ...
  <%--<TR>
    ...
    <TD><SELECT class=text name=type_id><OPTION selected 
        value="${house.HRect}">一室一厅</OPTION></SELECT></TD></TR>--%>
  <TR>
...
name=floorage value="${house.HRect}"></TD></TR>
  <%--<TR>
    ...
    <TD><INPUT id=add_action_price class=text type=text name=price value="${house.}"> </TD></TR>--%>
  <TR>
  ...
  <TR>
    ...
    <TD>区：<SELECT class=text name=districtId ><OPTION selected
        value="${house.HLoc1}">海淀区</OPTION></SELECT> 街：<SELECT class=text
        name=streetId><OPTION selected value="${house.HLoc2}">中关村大街</OPTION></SELECT> </TD></TR><!--
						<tr>
